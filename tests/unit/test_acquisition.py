...
def server():
    ...
    thread.start()
    yield f"http://127.0.0.1:{instance.server_port}/artifact"
    instance.shutdown()
    thread.join()

def test_acquisition_verifies_bytes(server, tmp_path: Path):
    payload = Handler.payload
    out = acquire_artifact(server, tmp_path / "artifact", expected_size=len(payload),
...
def test_acquisition_rejects_hash_and_leaves_no_partial(server, tmp_path: Path):
    with pytest.raises(ArtifactAcquisitionError):
        acquire_artifact(server, tmp_path / "artifact", expected_size=len(Handler.payload),
                         expected_sha256="0" * 64)
    assert not (tmp_path / "artifact").exists()
    assert not (tmp_path / "artifact.download").exists()

def test_acquisition_rejects_oversized_response(server, tmp_path: Path):
    with pytest.raises(ArtifactAcquisitionError):
        acquire_artifact(server, tmp_path / "artifact", expected_size=1,
                         expected_sha256="0" * 64)
