...
rm -rf "$STAGE" "$OUTDIR"
mkdir -p "$STAGE/DEBIAN" "$STAGE/opt/bke-digital-solutions/licensing-agent/agent" \
  ...
  "$STAGE/lib/systemd/system" "$OUTDIR"
...
dpkg-deb --build --root-owner-group "$STAGE" "$OUTDIR/bke-licensing-agent_${VERSION}_amd64.deb"
