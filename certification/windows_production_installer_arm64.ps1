...
$DurableMarker = Join-Path $DataRoot 'phase8-production-installer-marker.txt'
...
function Assert-Administrator {
    ...
    if (!$principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
        throw 'Phase 8 ARM64 certification must run from an elevated PowerShell.'
    }
}
...
function Wait-AgentAuthorized([int]$TimeoutSeconds = 90) {
    ...
    do {
        try {
            ...
            # Startup and migration may temporarily make the loopback API unavailable.
        }
...
function Assert-LoopbackOwnedByRuntime([int]$RuntimePid) {
    ...
    foreach ($listener in @($listeners)) {
        ...
        if ([int]$listener.OwningProcess -ne $RuntimePid) {
            throw "Agent port 43873 is not owned by the supervised Gen2 runtime. Listener PID=$($listener.OwningProcess), runtime PID=$RuntimePid"
        }
    }
}
...
function Assert-StableServiceIdentity {
    ...
    if ($record.StartMode -ne 'Auto') {
        throw "Agent service is not Automatic: $($record.StartMode)"
    }
...
if ($Mode -eq 'Prepare') {
    ...
    if ([IO.Path]::GetFileName($resolvedInstaller) -ne 'BKE-Licensing-Agent-2.0.0-Windows-arm64.exe') {
        throw "Unexpected Phase 8 installer filename: $resolvedInstaller"
    }
    ...
    if (!(Test-Path -LiteralPath $BridgeMarker -PathType Leaf)) {
        throw 'The Phase 7 certification bridge marker is missing. This is not the expected persistent Phase 7 ARM64 machine.'
    }
    if (!(Test-Path -LiteralPath $AgentDb -PathType Leaf)) {
        throw "Durable Agent database is missing before migration: $AgentDb"
    }
    if (!(Wait-AgentAuthorized 60)) {
        throw 'The durable Phase 7 signed runtime-bridge authorization fixture is not healthy before Phase 8 installation.'
    }

    $runtime = Wait-RuntimeProcess 60
    if (!$runtime) {
        throw 'Stable bootstrap is not supervising the Gen2 runtime before Phase 8 installation.'
    }
    Assert-LoopbackOwnedByRuntime ([int]$runtime.ProcessId)

    $markerValue = "phase8-arm64-production-installer|$([DateTimeOffset]::UtcNow.ToString('O'))"
    ...
    $expected = [ordered]@{
        ...
        system_type = [string]$computer.SystemType
        ...
        previous_service_sha256 = Get-FileSha256 $ServiceExe
        previous_runtime_sha256 = Get-FileSha256 $RuntimeExe
        bridge_marker_was_present = $true
    ...
    Write-Host 'Phase 8 ARM64 preparation: PASS'
...
if ($Mode -eq 'Install') {
    ...
    try {
        if (!(Test-Path -LiteralPath $ExpectedPath -PathType Leaf)) {
            throw "Phase 8 preparation evidence is missing: $ExpectedPath"
        }
        $expected = Get-Content -LiteralPath $ExpectedPath -Raw | ConvertFrom-Json
        if ($expected.schema -ne 'bke.production-installer-arm64.v1') {
            throw 'Unexpected Phase 8 preparation schema.'
        }
        ...
        if ((Get-FileSha256 $resolvedInstaller) -ne [string]$expected.installer_sha256) {
            throw 'Canonical ARM64 installer hash changed after preparation.'
        }
        ...
        if ($process.ExitCode -ne 0) {
            throw "Canonical ARM64 installer failed with exit code $($process.ExitCode). See $InstallerLog"
        }
        ...
        foreach ($required in @($ServiceExe, $RuntimeExe, $LicenseCenterExe, $UpdaterExe, $ProvisionerExe)) {
            if (!(Test-Path -LiteralPath $required -PathType Leaf)) {
                throw "Canonical installed asset is missing: $required"
            }
        }
        $result.checks['canonical_assets_present'] = $true
        ...
        if (Test-Path -LiteralPath $BridgeMarker) {
            throw 'Certification bridge marker leaked into the canonical Phase 8 installation.'
        }
        $result.checks['certification_marker_removed'] = $true

        if ((Test-Path -LiteralPath (Join-Path $InstallRoot 'service.rollback')) -or
            (Test-Path -LiteralPath (Join-Path $InstallRoot 'runtime.rollback'))) {
            throw 'Rollback staging leaked after a successful canonical Phase 8 installation.'
        }
        $result.checks['rollback_staging_discarded'] = $true

        if (!(Test-Path -LiteralPath $AgentDb -PathType Leaf)) {
            throw 'Durable Agent database was lost during canonical installation.'
        }
        if (!(Test-Path -LiteralPath $DurableMarker -PathType Leaf)) {
            throw 'Phase 8 durable ProgramData marker was lost during canonical installation.'
        }
        if ((Get-Content -LiteralPath $DurableMarker -Raw).Trim() -ne [string]$expected.durable_marker_value) {
            throw 'Phase 8 durable ProgramData marker changed during canonical installation.'
        }
        if (!(Wait-AgentAuthorized 90)) {
            throw 'Durable signed authorization did not recover through the canonical ARM64 installation.'
        }
        ...
        if (!$runtime) {
            throw 'Stable bootstrap is not supervising the Gen2 runtime after canonical ARM64 installation.'
        }
        ...
        $result['agent_db_sha256_after'] = Get-FileSha256 $AgentDb
    ...
    if ($result.status -ne 'PASS') {
        Write-Error "PHASE 8 ARM64 PRODUCTION-INSTALLER CERTIFICATION FAILED: $($result.error)"
        exit 1
    }

    Write-Host 'Canonical Phase 8 Windows ARM64 production-installer candidate: PASS'
    exit 0
}

if ($Mode -eq 'Collect') {
    if (!(Test-Path -LiteralPath $ResultPath -PathType Leaf)) {
        throw "Phase 8 result is not available: $ResultPath"
    }
    $result = Get-Content -LiteralPath $ResultPath -Raw | ConvertFrom-Json
    $result | ConvertTo-Json -Depth 10
    if ($result.status -ne 'PASS') {
        throw "Phase 8 ARM64 certification did not pass: $($result.error)"
    }
    exit 0
}
